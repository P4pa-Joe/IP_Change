...
- **Scan Network**: a ping/ARP sweep of the interface's subnet, with mDNS/DNS hostname resolution, shown as live-updating native submenu items (IP, MAC, hostname) — no separate window. Hovering a discovered host's submenu offers quick actions: open in HTTP/HTTPS, or connect via SSH/Telnet in Terminal.app.
...
`build-app.sh` reuses `Resources/AppIcon.icns` if present and sets the bundle identifier to `com.ipchange.app`. Edit `APP_VERSION` at the top of the script to change the version stamped into `CFBundleVersion` / `CFBundleShortVersionString`.
